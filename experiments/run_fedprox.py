"""
Federated Proximal (FedProx) experiment.

Implements federated learning with FedProx aggregation strategy.
"""
...
def run_fedprox_experiment(data_dir: str = "data/processed",
                          ...
                          alpha: float = 0.1):
    """
    Run FedProx federated learning experiment.
    ...
    """
    logger.info("="*60)
    logger.info("FEDERATED PROXIMAL (FedProx) EXPERIMENT")
    logger.info("="*60)
    ...
    strategy = FedProx(num_clients, mu=mu)
    ...
    with open(output_path / "metrics" / "fedprox_results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    logger.info(f"Results saved to {output_path / 'metrics' / 'fedprox_results.json'}")
    ...
    logger.info("="*60)
    logger.info("FedProx EXPERIMENT COMPLETED")
    logger.info("="*60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="FedProx federated learning experiment")
